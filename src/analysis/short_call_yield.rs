...
/// Lists cash-secured short-call yields using the live bid as premium received.
///
/// Assumption:
/// - The call expires worthless (no exercise), so the seller keeps the full premium.
/// - Annualized yield = premium_received_usd / strike_usd * 365 / days_to_expiry
pub struct ShortCallYieldAnalyzer;

impl ShortCallYieldAnalyzer {
    pub fn new() -> Self {
        Self
    }
    ...
    pub async fn scan(
        ...
        for inst in registry.get_all().await {
            ...
            // OTM call: strike must be above current price
            ...
            opportunities.push(Opportunity {
                strategy_type: "short_call_yield".to_string(),
                description: format!(
                    "{} SCALL K={:.0} >= ${:.0} | APY {:.1}%",
                    expiry_label,
                    inst.strike,
